...
<div class={[ 'filter-group', isOpen ? 'open' : null ].join(' ')}>
  <button onClick={ toggleState } class='header'>
  ...
  <div class='content' style={{ height: isOpen ? `${ fitterList.length * 2 + ( fitterList.length + 1 ) * .25 }rem` : `0rem` }}>
    { #each filters as filter }
      <button on:click={ () => toggleFilter(filter.value) } class={[ 'filter', filters.includes(filter.value) ? 'active' : null ].join(' ')}>
        { filter.title }
      </button>
    { /each }
  </div>
